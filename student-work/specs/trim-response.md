...
Student name: `korakris mangkasing`
...
1. If `Cm_alpha < 0` and the angle-of-attack disturbance is positive, `delta_Cm` should be `negative` because `a negative slope multiplied by a positive angle change yields a negative moment increment, creating a nose-down restoring pitching moment`.
2. If `Cm_alpha > 0` and the angle-of-attack disturbance is positive, the response should be `destabilizing` because `a positive slope multiplied by a positive angle change yields a positive (nose-up) moment increment that further increases the angle of attack`.
3. If `Cm_alpha = 0`, changing angle of attack should `produce no change in pitching moment (delta_Cm = 0), resulting in a neutral disturbance tendency and no unique trim angle`.
4. If `Cm0` is fixed and the magnitude of a nonzero `Cm_alpha` increases, the trim angle magnitude should `decrease because alpha_trim = -Cm0 / Cm_alpha`.
5. Doubling `disturbanceAlphaDeg` while holding `Cm_alpha` fixed should `double delta_Cm due to the linear relationship delta_Cm = Cm_alpha * delta_alpha_rad`.
...
Cm(alpha) = 0.04 + (-0.8 * 0.0499164) = 0.04 - 0.0399331 = 0.00006688

Trim angle:
alpha_trim_rad = -0.04 / -0.8 = 0.05 rad
alpha_trim_deg = 0.05 * (180 / pi) = 2.864789 deg

Disturbance response:
delta_Cm = -0.8 * 0.0349066 = -0.0279253

Expected classifications:
selected condition = not trimmed (since |Cm(2.86 deg)| = 6.688e-5 > 1e-6)
disturbance tendency = restoring  (since delta_alpha_rad * delta_Cm = 0.0349066 * (-0.0279253) = -0.0009748 < 0)
...
Expected Outputs:
- Cm(2.86 deg) = 6.688e-5 ± 1e-6 (dimensionless)
- alpha_trim_deg = 2.864789 ± 1e-4 deg
- delta_Cm = -0.0279253 ± 1e-6 (dimensionless)
...
```text
Change Cm_alpha from -0.8 1/rad to +0.8 1/rad while keeping Cm0 = 0.04, alpha = 2.86 deg, delta_alpha = +2.00 deg.
Expected Outputs:
- delta_Cm = +0.0279253 ± 1e-6 (dimensionless)
- disturbance tendency must change from "restoring" to "destabilizing" (since delta_alpha_rad * delta_Cm > 0)
- alpha_trim_deg = -2.864789 ± 1e-4 deg
```
...
```text
Set Cm_alpha = 0 1/rad, Cm0 = 0.04, alpha = 2.86 deg, delta_alpha = +2.00 deg.
Expected Outputs:
- alpha_trim_deg = "not available" (prevents division by zero)
- delta_Cm = 0.0 (dimensionless)
- disturbance tendency = neutral
- selected condition = not trimmed (Cm = 0.04 != 0)
```
...
```text
This feature supports deciding whether a given flight condition is trimmed and whether the static pitching-moment slope provides a restoring tendency following an angle-of-attack disturbance. It cannot establish dynamic stability, damping, handling qualities, or real-world controllability and safety.
```
